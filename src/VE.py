...
def new_generate_beta(lambda2):
    # you could set a small gap to aacclerate computing!（您可以设置一个小的间隙来加速计算）
    gap = 100
...
def VertexEnt(G, alpha,belta=None, perturb_strategy='default', printLog=False):
    """
    近似计算的方法计算节点纠缠度
    :param G:
    :return:
    ...
    for i in range(0, len(belta)):
        ...
        S[i] = 1 / 1-alpha * np.log2(sum((np.exp(t) / Z_)) ** alpha)
    ...
    #             neibour = sorted(neibour)
    #             dA = weight - A[neibour][:, neibour]
    #             dA = dA - np.diag([weight] * (kx + 1))[:, neibour]  # 使用广播规则扩展 dA 的形状
    #             dL = np.diag(np.array(sum(dA)).flatten()) - dA
    #             t__ = eigVector[neibour][:, np.newaxis].T @ dL @ eigVector[neibour]  # 使用广播规则对矩阵进行乘法操作
    #             lambda_ral[v_id] = eigValue + np.squeeze(t__)  # 将 t__ 扁平化后与 eigValue 相加
    # elif perturb_strategy == 'remove':
    #     for v_id in tqdm(range(0, N), desc="Computing eigenValues for removed networks", unit="node"):
    #         neibour = list(G.neighbors(v_id))
    #         pt_A = copy.deepcopy(A)
    ...
    #         dA = dA[neibour][:, neibour]
    #         dL = np.diag(np.array(sum(dA)).flatten()) - dA
    #         t__ = eigVector[neibour][:, np.newaxis].T @ dL @ eigVector[neibour]
    #         lambda_ral[v_id] = eigValue + np.squeeze(t__)
    ...
    for x in tqdm(range(0, N), desc="Searching minium entanglement", unit="node"):
        ...
        for i in range(0,len(belta)):
            ...
            E[i, x] = 1 / 1 - alpha * np.log2(sum((np.exp(t) / Z_)) ** alpha) - S[i]
    VE = np.min(E, axis=0)
...
def parse_args():
    ...
    # parser.add_argument('--net', default='email', help='graph name')
    parser.add_argument('--device', type=str, default='cpu')  # 这个比较重要，是看CPU还是GPU跑
    parser.add_argument('--net', type=str, default='Amazon-Computers')
...
if __name__ == '__main__':
    ...
    # for netname in ['email']:#'collaboration','grid','GrQC','facebook','figeys','ham','hep'，'hepth','RU','ES','lastfm','FR','DE','ENGB',
    for netname in ['Amazon-Computers']:
        ...
        if not os.path.exists(ve_value_path):
            VE = VertexEnt(G, alpha=args.alpha, belta=args.belta, perturb_strategy=args.perturb_strategy)
